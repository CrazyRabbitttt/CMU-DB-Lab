...
void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
  ...
  if (counter_.count(frame_id) == 0) {
    ...
    if (history_map_.count(frame_id) != 0) {
      ...
      history_map_.erase(frame_id);                      // 从 容器中清除数据
      if (counter_[frame_id] >= static_cast<int>(k_)) {  // 能够直接放到 cache list 了
...
void LRUKReplacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
  ...
  if (history_map_.count(frame_id) == 0 && cache_mp_.count(frame_id) == 0) {
    THROWERROR("The frame_id  is invalid!");
  }
 ...
 * TODO(P1): Add implementation
 *
 * @brief Remove an evictable frame from replacer, alo
 * ng with its access history.
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
  ...
  if (history_map_.count(frame_id) > 0) {  // 在 history list 中
    ...
    DecrementEvictableSize();
    history_linked_list_.erase(cache_mp_[frame_id]);
