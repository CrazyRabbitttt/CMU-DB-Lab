    ...
    : global_depth_(1), bucket_size_(bucket_size), num_buckets_(2)
{
  ...
  dir_.push_back(std::make_shared<Bucket>(bucket_size_));       // dir_ 中插入一个 bucket        目前目录的 bit 数目为1，对应两个
}
...
void ExtendibleHashTable<K, V>::Insert(const K &key, const V &value) {
  ...
  size_t index = IndexOf(key);
  std::shared_ptr<Bucket> bucket = dir_[index];
  int local_depth = bucket->GetDepth(), global_depth = GetGlobalDepth();
  if (bucket->IfExistsThenUpdate(key, value)) return ;
  ...
  if (local_depth < global_depth) {   // 只能是说 桶进行分裂， 但是目录不会进行一个扩张的操作
    ...
    auto pairs = bucket->GetItems(); pairs.push_back(std::make_pair(key, value));
    bucket->ClearTheBucket();
    int mask = (1 << local_depth);
    int target = index | mask;            // 最高位的 bit 取 1
    dir_[target] = std::make_shared<Bucket>(bucket_size_, local_depth+1);    // 创建一个新的 bucket 【bucket分裂】
    RedistributeBucket(pairs);
  } else {      // local_depth == global depth
    IncrementGlobalDepth();         // 1. increment the global depth
    bucket->IncrementDepth();       // 2. increment the local depth
    int n = dir_.size();
    dir_.resize(n * 2);             // 3. double the size of dir
    // 下面的操作同上面是一样的， 只不过是说 最高 bit 位为1的时候进行一个映射
    for (int i = 0; i < n; i++) { dir_[i + (1 << local_depth)] = dir_[i]; }       // 相同的映射， 只有 bucket 进行分裂的时候才创建新的 shared_ptr
...
template <typename K, typename  V>
auto ExtendibleHashTable<K, V>::RedistributeBucket(std::list<std::pair<K, V>> &pairs) -> void {
  // 获得了 list, 直接进行 hash 操作就行了
  for (auto& it : pairs) {
...
auto ExtendibleHashTable<K, V>::Bucket::Find(const K &key, V &value) -> bool {
  ...
  if (index == list_.end()) { return false; }
  value = (*index).second;
  return true;
}
...
auto ExtendibleHashTable<K, V>::Bucket::Insert(const K &key, const V &value) -> bool {
  auto index = std::find_if(list_.begin(), list_.end(), [&](const std::pair<K, V> v) { return v.first == key; });
  if (index != list_.end()) { (*index).second = value; return true; }        // 1.如果说存在 key 的话，直接更新 value
  if (IsFull()) return false;                                                // 2.如果说 bucket is full, do nothing & return false
